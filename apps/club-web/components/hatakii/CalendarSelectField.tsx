...
export const CalendarSelectField = ({
  ...
  <div className="space-y-2">
    ...
    <div className="relative">
      ...
      >
        {options.map((opt: string | SelectOption, index: number) => {
          const isObject = typeof opt !== 'string';
          const displayLabel = isObject ? opt.l : opt;
          const displayValue = isObject ? opt.v : opt;

          return (
            <option
              key={isObject ? opt.v : `opt-${index}`}
              value={displayValue}
              className="bg-[#050c1f]"
            >
              {displayLabel}
